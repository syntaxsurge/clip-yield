...
- FFmpeg WASM assets (core + class worker) live in `public/ffmpeg` and are synced from `@ffmpeg/core` + `@ffmpeg/ffmpeg` via `scripts/copy-ffmpeg-core.mjs` (runs on `postinstall` and `pnpm ffmpeg:sync-core`)
